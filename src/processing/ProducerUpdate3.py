...
class ProducerUpdate3:
    ...
    @profile(stream=open('q_mp_run_bm2.log', 'w+'))
            ...
            if not self.queue.empty():
                ...
                if len(ml_rdm_queue_list) == STEP_TIME + 1:
                    threading.Thread(
                        name="ML2_" + '_' + str(i),
                        target=self.run_ml,
                        args=(STEP_TIME, count_step_time, ml_rdm_queue_list.copy(), ml_rdm_queue_date.copy())
                    ).start()
                    i += 1
                    del ml_rdm_queue_date[:]
                    del ml_rdm_queue_list[:]
                ...
                if len(fall_rdm_queue_list) == COUNT_VEL * det_count_thereshold:
                    threading.Thread(
                        ...
                        args=(COUNT_VEL, det_count_thereshold, fall_rdm_queue_list.copy(), fall_rdm_queue_date.copy())
                    ).start()
                    del fall_rdm_queue_date[:]
                    del fall_rdm_queue_list[:]
                    j += 1
                ...
                if len(move_rdm_queue_list) == COUNT_MOVE * det_count_move:
                    threading.Thread(
                        ...
                        args=(COUNT_MOVE, det_count_move, move_rdm_queue_list.copy(), "move_" + '_' + str(mm), move_rdm_queue_date.copy())
                    ).start()
                    del move_rdm_queue_date[:]
    ...
    @profile(stream=open('q_mp_run_ml.log', 'w+'))
    ...
    @profile(stream=open('q_mp_run_fall.log', 'w+'))
    ...
    @profile(stream=open('q_mp_run_move.log', 'w+'))
    ...
    @profile(stream=open('q_mp_final_fall.log', 'w+'))
    ...
    @profile(stream=open('q_mp_bed_detection.log', 'w+'))
